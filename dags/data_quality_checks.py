...
def validate_data_quality(**context):
    ...
    try:
        ...
        task_instance.xcom_push(
            key="validation_success", value=result_dict.get("success")
        )
        task_instance.xcom_push(
            key="validation_stats", value=result_dict.get("statistics")
        )

        # Check if validation passed
        if not result_dict.get("success", False):
            failed_count = result_dict.get("statistics", {}).get(
...
def check_validation_status(**context):
    ...
    success = task_instance.xcom_pull(
        task_ids="validate_data_quality", key="validation_success"
    )

    stats = task_instance.xcom_pull(
        task_ids="validate_data_quality", key="validation_stats"
    )
