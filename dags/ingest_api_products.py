...
def validate_product_data(**context):
    ...
    invalid_prices = [p for p in products if not isinstance(p.get('price'), (int, float)) or p.get('price', 0) <= 0]
...
def enrich_product_data(**context):
    ...
    context['ti'].xcom_push(key='enriched_products', value=json.dumps(enriched_products))
...
with DAG(
    ...
    task_get_date >> task_fetch >> task_validate >> task_enrich >> task_load >> task_summary
