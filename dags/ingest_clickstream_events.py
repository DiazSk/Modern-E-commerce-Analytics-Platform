...
def read_and_filter_events(**context):
    ...
    context['ti'].xcom_push(key='events_data', value=df.to_json(orient='records', date_format='iso'))
...
def validate_events(**context):
    ...
    - Timestamps
    """
    events_json = context['ti'].xcom_pull(
        key='events_data',
        task_ids='read_events'
    )
    ...
    valid_event_types = ['page_view', 'add_to_cart', 'remove_from_cart', 'purchase', 'search']
...
def partition_and_upload_to_s3(**context):
    ...
    logging.info(f"Partitioning {len(df)} events into {len(date_groups)} date partitions")
    ...
    for (year, month, day), group_df in date_groups:
        ...
        logging.info(f"   Uploading {len(group_df)} events to: s3://{S3_RAW_BUCKET}/{s3_key}")
...
def log_summary(**context):
    ...
    total_events = context['ti'].xcom_pull(
        key='total_events',
        task_ids='upload_to_s3'
    ) or 0
    
    partition_count = context['ti'].xcom_pull(
        key='partition_count',
        task_ids='upload_to_s3'
    ) or 0
