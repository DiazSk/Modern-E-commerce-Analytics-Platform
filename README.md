...
| Layer | Tools |
|---|---|
| Source | REES46 clickstream on Kaggle, downloaded with the Kaggle API (`kaggle` 1.6.17) |
| Platform | Databricks Free Edition: serverless jobs, Unity Catalog (catalog, schemas, volume), Serverless Starter SQL warehouse |
| Orchestration | Databricks Workflows job (`ingest → transform → dbt_build`), defined as a Databricks Asset Bundle and deployed with the Databricks CLI |
| Storage | Delta Lake, partitioned by `event_date`, month-scoped `replaceWhere` overwrites |
| Processing | PySpark (Spark 3.5.3) |
| Transformation | dbt Core 1.12.5: `dbt-databricks` in production, `dbt-spark[session]` 1.11.0 in CI; incremental merge models |
| Testing | pytest; dbt generic, unit and singular tests |
| CI | GitHub Actions (Python 3.11, Java 17) running local Spark + `delta-spark` 3.2.1 on synthetic fixtures |
| Code quality | black, isort, flake8, pre-commit |
| Analysis | SQL on Databricks SQL through the Statement Execution API (`analysis/run_queries.py`); 95% Wald confidence intervals |
| Visualization | matplotlib charts; a static HTML/CSS/JavaScript dashboard with hand-built SVG charts, generated by Python and hosted on GitHub Pages |
